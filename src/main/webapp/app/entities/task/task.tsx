  ...
  return (
    <div>
      <h2 id="task-heading" data-cy="TaskHeading">
        <Translate contentKey="crmApp.task.home.title">Tasks</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="crmApp.task.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/task/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="crmApp.task.home.createLabel">Create new Task</Translate>
          </Link>
        </div>
      </h2>
        ...
        {taskList && taskList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="crmApp.task.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('due_Date')}>
                  ...
                  <FontAwesomeIcon icon={getSortIconByFieldName('due_Date')} />
                </th>
                <th className="hand" onClick={sort('status')}>
                  <Translate contentKey="crmApp.task.status">Status</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('status')} />
                </th>
                ...
                <th className="hand" onClick={sort('related_to')}>
                  ...
                  <FontAwesomeIcon icon={getSortIconByFieldName('related_to')} />
                </th>
                <th className="hand" onClick={sort('task_Owner')}>
                  <Translate contentKey="crmApp.task.task_Owner">Task Owner</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('task_Owner')} />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {taskList.map((task, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{task.subject}</td>
                  <td>{task.due_Date ? <TextFormat type="date" value={task.due_Date} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  <td>{task.status}</td>
                  <td>{task.priority}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="crmApp.task.home.notFound">No Tasks found</Translate>
            </div>
          )
        )}
      </div>
